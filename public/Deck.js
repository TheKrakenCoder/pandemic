...
class Deck {
  ...
  show(x, y, facedown) {
    ...
    // We only need to show one card.  Spreading is handled elsewhere.
    let len = this.cards.length-1;
    this.cards[len].x = x;
    this.cards[len].y = y;
    this.cards[len].show(facedown);

    // draw how many cards are in the deck
    let sz = 24*m_s;
    textSize(sz);
    stroke(0), fill(255);
    text(len+1, x+this.cw-1.5*sz, y+sz);

  }

  spread() {
    let xwrap = 6;
    // if this deck is horizontal, change the wrap
    if (this.cw > this.ch) {
      xwrap = 4;
    }
    let x = 0, y = 0;
